...
from grain_growth_model.utils.io import load_config_module, parse_arguments, generate_simulation_report
...
def main():
    ...
    for layer_idx in range(len(config.THERMAL_HISTORY)):
        ...
        layer_meltpool_description = config.THERMAL_HISTORY[layer_idx]
        ...
        extended_interface_coords, extended_interface_oris, extended_interface_ids, grains_generated = initialize_interface_grains(
    ...
    tracking['tot. simulation time'] = round(time.time() - time_start_simulation, 2)

    # Writing reports
    generate_simulation_report(
        data=tracking, 
        config_module=config, 
        output_file=os.path.join(working_paths["data"], "report_simulation.txt"),
        )
